...
func (h *Handle) linkModify(link Link, flags int) error {
	...
	log.Debugf("in link modify")
	base := link.Attrs()
	log.Debugf("link attrs done")
	...
	if isTuntap {
		...
		log.Debugf("before ensure index")

		h.ensureIndex(base)

		log.Debugf("before master index")
		...
		log.Debugf("Return successfully 2")
		return nil
	}
